...
<div class="main-content-inner">
    <div class="main-content-wrap">
        ...
        <div class="wg-box">
            <form action="{{ route('categories.update', $category->id) }}" method="POST" enctype="multipart/form-data" class="form-new-product form-style-1">
                @csrf
                @method('PUT')

                <fieldset class="name">
                    <div class="body-title">Category name <span class="tf-color-1">*</span></div>
                    <input class="flex-grow" type="text" name="name" value="{{ old('name', $category->name) }}">
                    ...
                    <div class="upload-image flex-grow">
                        <div class="item up-load" id="uploadBox" style="position:relative; overflow:hidden;">
                            <label class="uploadfile" for="myFile">
                                <span class="icon" id="uploadIcon" style="{{ $category->image ? 'display:none' : '' }}">
                                    <i class="icon-upload-cloud"></i>
                                </span>
                                <span class="body-text" id="uploadText" style="{{ $category->image ? 'display:none' : '' }}">
                                    Upload new image to replace <span class="tf-color">click to browse</span>
                                </span>
                            ...
                            <img id="imagePreview"
                                 ...
                                 alt="Preview"
                                 style="{{ $category->image ? 'display:block;' : 'display:none;' }} position:absolute; top:50%; left:50%; transform:translate(-50%,-50%); max-width:90%; max-height:90%; object-fit:contain; border-radius:12px;">
                        </div>
                    </div>
                ...
                <fieldset>
                    ...
                    <textarea class="flex-grow" name="description">{{ old('description', $category->description) }}</textarea>
                </fieldset>
